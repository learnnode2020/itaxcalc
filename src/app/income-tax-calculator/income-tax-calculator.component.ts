...
export class IncomeTaxCalculatorComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.other_income = !Number.isNaN(parseFloat(this.f['other_income'].value))
      ? parseFloat(this.f['other_income'].value)
      : 0;
    this.hra_deduction =
      this.f['hra_deduction'].value != null &&
      this.f['hra_deduction'].value != undefined
        ? parseFloat(this.f['hra_deduction'].value)
        : 0;
    this.d80c_deduction = !Number.isNaN(
      parseFloat(this.f['d80c_deduction'].value)
    )
      ? parseFloat(this.f['d80c_deduction'].value)
      : 0;

    this.d80ccd1b_deduction = !Number.isNaN(
      parseFloat(this.f['d80ccd1b_deduction'].value)
    )
      ? this.f['d80ccd1b_deduction'].value
      : 0;
    this.d80ccd2_deduction = !Number.isNaN(
      parseFloat(this.f['d80ccd2_deduction'].value)
    )
      ? parseFloat(this.f['d80ccd2_deduction'].value)
      : 0;
    this.d80d_deduction = !Number.isNaN(
      parseFloat(this.f['d80d_deduction'].value)
    )
      ? parseFloat(this.f['d80d_deduction'].value)
      : 0;
    this.prof_tax = !Number.isNaN(parseFloat(this.f['prof_tax'].value))
      ? parseFloat(this.f['prof_tax'].value)
      : 0;
    this.d80tta_deduction = !Number.isNaN(
      parseFloat(this.f['d80tta_deduction'].value)
    )
      ? parseFloat(this.f['d80tta_deduction'].value)
      : 0;
    this.d24interest = !Number.isNaN(parseFloat(this.f['d24interest'].value))
      ? parseFloat(this.f['d24interest'].value)
      : 0;
    this.d80EEinterest = !Number.isNaN(
      parseFloat(this.f['d80EEinterest'].value)
    )
      ? parseFloat(this.f['d80EEinterest'].value)
      : 0;

    this.d80E_educationloaninterest = !Number.isNaN(
      parseFloat(this.f['d80E_educationloaninterest'].value)
    )
      ? parseFloat(this.f['d80E_educationloaninterest'].value)
      : 0;
    this.tcs_tax = !Number.isNaN(parseFloat(this.f['tcs_tax'].value))
      ? parseFloat(this.f['tcs_tax'].value)
      : 0;
    this.tds_tax = !Number.isNaN(parseFloat(this.f['tds_tax'].value))
      ? parseFloat(this.f['tds_tax'].value)
      : 0;
    ...
    this.other_income_after_80tta =
      this.other_income >= this.d80tta_deduction
        ? this.other_income - this.d80tta_deduction
        : 0;
    this.total_deductions_old =
      this.std_deduction +
      this.prof_tax +
      this.hra_deduction +
      this.d80c_deduction +
      this.d80ccd1b_deduction +
      this.d80ccd2_deduction +
      this.d80d_deduction +
      this.d24interest +
      this.d80E_educationloaninterest;

    this.total_deductions_new = this.std_deduction + this.d80ccd2_deduction;

    this.calculated_netIncome_oldregime = Math.floor(
      this.netIncome + this.other_income_after_80tta - this.total_deductions_old
    );
    this.calculated_netIncome_newregime = Math.floor(
      this.netIncome + this.other_income - this.total_deductions_new
    );
    if (this.calculated_netIncome_newregime < 0)
      this.calculated_netIncome_oldregime = 0;
    if (this.calculated_netIncome_newregime < 0)
      this.calculated_netIncome_newregime = 0;

    this.totalTax_old = this.taxCalculatorService.taxCalculator_old_regime(
    ...
    if (this.totalTax_new < 0) this.totalTax_new = 0;
    if (this.totalTax_old < 0) this.totalTax_old = 0;

    this.eligible_tax_rebate_old = 0;
    this.eligible_tax_rebate_new = 0;
    ...
    if (this.tax_new_withcess < 0) this.tax_new_withcess = 0;

    if (this.tax_old_withcess < 0) this.tax_old_withcess = 0;

    this.tax_new_withcess_less_tds =
      this.tax_new_withcess - this.tcs_tax - this.tds_tax;
    this.tax_old_withcess_less_tds =
      this.tax_old_withcess - this.tcs_tax - this.tds_tax;
    ...
    this.neworoldRegime = false;
    if (
      this.tax_new_withcess_less_tds == 0 &&
      this.tax_old_withcess_less_tds == 0
    )
      debugger;
    this.neworoldRegime = false;
